...
def eval_submission(submission_csv_path,
    ...
    for dataset in gt_dict:
        ...
        for scene in gt_dict[dataset]:
            assert scene in submission_dict[
                dataset], f'Unknown scene: {dataset}->{scene}'
            for image in gt_dict[dataset][scene]:
                assert image in submission_dict[dataset][
                    scene], f'Unknown image: {dataset}->{scene}->{image}'
        ...
        for scene in gt_dict[dataset]:
            ...
            for i in range(len(images)):
                for j in range(i + 1, len(images)):
                    ...
                    dR_gt, dT_gt = compute_dR_dT(gt_i.rotmat, gt_i.tvec,
                                                 gt_j.rotmat, gt_j.tvec)

                    pred_i = submission_dict[dataset][scene][images[i]]
                    pred_j = submission_dict[dataset][scene][images[j]]
                    dR_pred, dT_pred = compute_dR_dT(pred_i.rotmat,
                                                     pred_i.tvec,
                                                     pred_j.rotmat,
                                                     pred_j.tvec)
